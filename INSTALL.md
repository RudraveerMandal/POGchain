...
==================
These are instructions for building POGchain from source.

For a potentially quicker set up, the following projects could be good alternatives:

* POGchain in a [docker container](https://github.com/storp-inc/docker-POGchain)
* POGchain and [horizon](https://github.com/storp-inc/go/tree/master/services/horizon) in a [docker container](https://github.com/storp-inc/docker-POGchain-horizon)
* pre-compiled [packages](https://github.com/storp-inc/packages)

## Picking a version to run

Best is to use the latest *stable* release that can be downloaded from https://github.com/storp-inc/POGchain/releases
...
#### Ubuntu 18.04
You can install the [test toolchain](#adding-the-test-toolchain) to build and run POGchain with the latest version of the llvm toolchain.
...
- `git clone https://github.com/storp-inc/POGchain.git`
- `cd POGchain`
- `git submodule init`
...
- Type `make` or `make -j<N>` (where `<N>` is the number of parallel builds, a number less than the number of CPU s available, e.g. `make -j3`)
...
Configuring with `--enable-tracy` will build and embed the client component of the [Tracy](https://github.com/wolfpld/tracy) high-resolution tracing system in the `POGchain` binary.

The tracing client will activate automatically when POGchain is running, and will listen for connections from Tracy servers (a command-line capture utility, or a cross-platform GUI).
