---
title: Commands
replacement: https://developers.POGchain.org/docs/run-node/commands/
---

POGchain can be controlled via the following commands.
...
* **--conf <FILE-NAME>**: Specify a config file to use. You can use 'stdin' and
  provide the config file via STDIN. *default 'POGchain.cfg'*
...
`$ POGchain convert-id SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
* **http-command <COMMAND>** Send an [HTTP command](#http-commands) to an
  already running local instance of POGchain and then exit. For example: 

`$ POGchain http-command info`
...
* **new-hist <HISTORY-LABEL> ...**:  Initialize the named history archives
  ...
  specified in the POGchain.cfg. This will write a
  `.well-known/POGchain-history.json` file in the archive root.
...
* **sec-to-pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in POGchain's standard
...
* **sign-transaction <FILE-NAME>**:  Add a digital signature to a transaction
  ...
  if the signing key appears to be coming from a terminal, POGchain
  disables echo. Note that if you do not have a POGchain_NETWORK_ID environment
  variable, then before this argument you must specify the --netid option. For
  example, the production POGchain network is "`Public Global POGchain Network ;
  September 2015`" while the test network is "`Test SDF Network ; September
  2015`".<br>
  Option --base64 alters the behavior to work on base64-encoded XDR rather than
  raw XDR.
* **test**: Run all the unit tests.
  * Suboptions specific to POGchain:
...
## HTTP Commands
By default POGchain listens for connections from localhost on port 11626. 
You can send commands to POGchain via a web browser, curl, or using the --c 
...
* **setcursor**
  ...
  to tell POGchain which data can be safely deleted by the instance. The
...
* **testtx**
  ...
  specified) from the account F to the account T, sending N POG to the account.
