...
fuzzer with POGchain. Support for this is still preliminary but it has
...
perform well in area (a) and with simple modifications we made to POGchain, (b)
too. However, we've spent more energy with (c) for the `tx` fuzz mode, modifying
POGchain such that it is more deterministic -- caches are cleared and randomness
...
## Building an instrumented POGchain
...
to `overlay` and then `make fuzz`; this will do the following:
  ...
  - Run `POGchain gen-fuzz fuzz-testcases/fuzz$i.xdr` ten thousand times
  ...
  - Run `afl-fuzz` on `POGchain fuzz`, using those corpus directories

If `POGchain fuzz` (or `afl-fuzz`) produces output such as 'Warning: AFL++
...
by feeding it to an instance of `POGchain fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`POGchain` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`POGchain` in a different directory with `--enable-asan`, or valgrind, in
...
the tests). Then you can run `afl-fuzz -m 500 -M main -t 250 -i min-testcases -o fuzz-findings ./POGchain fuzz --ll ERROR --process-id 0 --mode=${FUZZER_MODE} @@` 
with multiple versions of , which will use the `min-testcases` directory previously created. If 
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `POGchain` itself, not libsodium,
  ...
  - Make startup-modes at different points in the process: instantiate an
    ...
    `POGchainMessage`s. Let the fuzzer generte bucket ledger entries, and try to
