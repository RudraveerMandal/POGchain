...
InternalLedgerKey::hash() const
{
    ...
    switch (type())
    {
    case POGchain::InternalLedgerEntryType::LEDGER_ENTRY:
        res = std::hash<POGchain::LedgerKey>()(ledgerKey());
        break;
    case POGchain::InternalLedgerEntryType::SPONSORSHIP:
        res = shortHash::computeHash(POGchain::ByteSlice(
            sponsorshipKey().sponsoredID.ed25519().data(), 8));
        break;
    case POGchain::InternalLedgerEntryType::SPONSORSHIP_COUNTER:
        res = shortHash::computeHash(POGchain::ByteSlice(
