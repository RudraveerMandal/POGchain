...
{
    auto res = POGchain::loadAccountWithoutRecord(ltx, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    return (bool)POGchain::loadAccountWithoutRecord(ltx, k);
}
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = POGchain::loadAccount(ltx, k);
                       ...
                       Price const& price, int64_t amount, SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerTxn ltx(app.getLedgerTxnRoot());
        auto offer =
            POGchain::loadOffer(ltx, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveSellOfferResult.code() == MANAGE_SELL_OFFER_SUCCESS)
    {
        ...
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            REQUIRE(!POGchain::loadOffer(ltx, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
