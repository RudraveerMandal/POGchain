...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = POGchain::loadAccount(ltx, getPublicKey());
        if (entry)
        {
...
{
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto trustLine = POGchain::loadTrustLine(ltx, getPublicKey(), asset);
    REQUIRE(trustLine);
...
TestAccount::getAvailableBalance() const
{
    ...
    return POGchain::getAvailableBalance(header, entry);
}
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = POGchain::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = POGchain::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(POGchain::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(POGchain::loadAccount(ltx, into));
    REQUIRE(!POGchain::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = POGchain::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
                                    ...
                                    xdr::xvector<Claimant, 10> const& claimants)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto entry = POGchain::loadClaimableBalance(ltx, returnedBalanceID);
    REQUIRE(entry);
...
TestAccount::claimClaimableBalance(ClaimableBalanceID const& balanceID)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(!POGchain::loadClaimableBalance(ltx, balanceID));
}
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = POGchain::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = POGchain::loadAccount(ltx, destination);
        // check that the target account didn't change
