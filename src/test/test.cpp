...
bool force_sqlite = (std::getenv("POGchain_FORCE_SQLITE") != nullptr);
    ...
    if (!cfgs[instanceNumber])
    {
        if (gTestRoots.empty())
        {
            gTestRoots.emplace_back(
                fmt::format("POGchain-test-{}", gBaseInstance));
        }
...
runTest(CommandLineArgs const& args)
{
    ...
    parser |= Catch::clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "POGchain can run tests concurrently");
    parser |=
    ...
    // not mixed with POGchain logging.
    Logging::setFmt("<test>");
    Logging::setLogLevel(logLevel, nullptr);
    // use base instance for logging as we're guaranteed to not have conflicting
    // instances of POGchain running at the same time with the same base
    // instance
    auto logFile = fmt::format("POGchain{}.log", gBaseInstance);
    Logging::setLoggingToFile(logFile);
    Logging::setLogLevel(logLevel, nullptr);

    LOG_INFO(DEFAULT_LOG, "Testing POGchain {}", POGchain_CORE_VERSION);
