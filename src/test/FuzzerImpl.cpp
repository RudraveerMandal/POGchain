...
struct xdr_fuzzer_compactor
{
    ...
    typename std::enable_if<
        (!std::is_same<POGchain::AccountID, T>::value &&
         !std::is_same<POGchain::MuxedAccount, T>::value &&
         !std::is_same<POGchain::Asset, T>::value &&
         !std::is_same<POGchain::AssetCode, T>::value &&
         !std::is_same<POGchain::ClaimableBalanceID, T>::value &&
         !std::is_same<POGchain::LedgerKey, T>::value) &&
    ...
    template <typename T>
    typename std::enable_if<std::is_same<POGchain::AccountID, T>::value>::type
    operator()(T const& pk)
    {
        // convert public key 1 byte
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(pk.ed25519());
        put_bytes(&b, 1);
    }
    template <typename T>
    typename std::enable_if<std::is_same<POGchain::MuxedAccount, T>::value>::type
    operator()(T const& m)
    {
        // convert MuxedAccount -> 1 byte (same than an AccountID)
        auto const& ed25519 = (m.type() == POGchain::KEY_TYPE_ED25519)
                                  ? m.ed25519()
                                  : m.med25519().ed25519;
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(ed25519);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<POGchain::Asset, T>::value>::type
    operator()(T const& asset)
    {
        // Convert Asset to 1 byte.
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(asset);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<POGchain::AssetCode, T>::value>::type
    operator()(T const& code)
    {
        // Convert AssetCode to 1 byte.
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(code);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<POGchain::ClaimableBalanceID, T>::value>::type
    operator()(T const& balanceID)
    {
        ...
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(balanceID);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<POGchain::LedgerKey, T>::value>::type
    operator()(T const& key)
    {
        ...
        check(1);
        auto b = POGchain::FuzzUtils::getShortKey(key);
        put_bytes(&b, 1);
    }
};
...
{
    POGchain::FuzzUtils::StoredLedgerKeys mStoredLedgerKeys;
    POGchain::FuzzUtils::StoredPoolIDs mStoredPoolIDs;
    std::uint8_t const* mCur;
    std::uint8_t const* const mEnd;

    xdr_fuzzer_unpacker(
        POGchain::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
        POGchain::FuzzUtils::StoredPoolIDs const& storedPoolIDs,
    ...
    typename std::enable_if<
        (!std::is_same<POGchain::AccountID, T>::value &&
         ...
         !std::is_same<POGchain::AssetCode, T>::value &&
         !std::is_same<POGchain::ClaimableBalanceID, T>::value &&
         !std::is_same<POGchain::LiquidityPoolDepositOp, T>::value &&
         !std::is_same<POGchain::LiquidityPoolWithdrawOp, T>::value &&
         !std::is_same<POGchain::LedgerKey, T>::value) &&
        (xdr_traits<T>::is_class || xdr_traits<T>::is_container)>::type
    operator()(T& t)
    {
    ...
    operator()(T& pk)
    {
        ...
        POGchain::FuzzUtils::setShortKey(pk, v);
    }
    template <typename T>
    typename std::enable_if<std::is_same<POGchain::MuxedAccount, T>::value>::type
    operator()(T& m)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        POGchain::FuzzUtils::setShortKey(m.ed25519(), v);
    }
    ...
    operator()(T& asset)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        asset = POGchain::FuzzUtils::makeAsset(v);
    }
    ...
    operator()(T& code)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        code = POGchain::FuzzUtils::makeAssetCode(v);
    }
    ...
    operator()(T& balanceID)
    {
        check(1);
        std::uint8_t v = get_byte();
        POGchain::LedgerKey key;
        POGchain::FuzzUtils::setShortKey(mStoredLedgerKeys, key, v);
        ...
        if (key.type() == POGchain::CLAIMABLE_BALANCE)
        {
            balanceID = key.claimableBalance().balanceID;
        }
        else
        {
            balanceID.type(POGchain::CLAIMABLE_BALANCE_ID_TYPE_V0);
            balanceID.v0()[0] = v;
        }
    }
    ...
    template <typename T>
    typename std::enable_if<
        std::is_same<POGchain::LiquidityPoolDepositOp, T>::value>::type
    operator()(T& depositOp)
    {
        check(1);
        auto v = get_byte();
        POGchain::FuzzUtils::setShortKey(mStoredPoolIDs,
        ...
        depositOp.minPrice = POGchain::Price{minN, minD};
        depositOp.maxPrice = POGchain::Price{maxN, maxD};
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<POGchain::LiquidityPoolWithdrawOp, T>::value>::type
...
xdr_from_fuzzer_opaque(
    POGchain::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
    POGchain::FuzzUtils::StoredPoolIDs const& storedPoolIDs, Bytes const& m,
...
template <>
void
generator_t::operator()(POGchain::PublicKey& t) const
{
    ...
    static_assert(NUMBER_OF_ACCOUNT_IDS_TO_GENERATE >
                      POGchain::FuzzUtils::NUMBER_OF_PREGENERATED_ACCOUNTS,
                  "Range of generated accounts too small");
    POGchain::FuzzUtils::setShortKey(
        t.ed25519(), static_cast<uint8_t>(POGchain::rand_uniform<int>(
...
bool
isBadOverlayFuzzerInput(POGchainMessage const& m)
{
...
TransactionFuzzer::storeSetupLedgerKeysAndPoolIDs(AbstractLedgerTxn& ltx)
{
    ...
    POGchain::FuzzUtils::generateStoredLedgerKeys(firstGeneratedLedgerKey,
...
TransactionFuzzer::reduceNativeBalancesAfterSetup(AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& param : accountParameters)
    {
        ...
        auto ae = POGchain::loadAccount(ltx, account);
    ...
    AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& trustLine : trustLineParameters)
    {
        ...
        // issuer.
        auto tle = POGchain::loadTrustLine(ltx, account, asset);
...
{
    reinitializeAllGlobalStateWithSeed(1);
    POGchain::FuzzUtils::generateStoredLedgerKeys(mStoredLedgerKeys.begin(),
                                                 mStoredLedgerKeys.end());
...
OverlayFuzzer::genFuzz(std::string const& filename)
{
    ...
    out.exceptions(std::ios::failbit | std::ios::badbit);
    out.open(filename, std::ofstream::binary | std::ofstream::trunc);
    autocheck::generator<POGchainMessage> gen;
    POGchainMessage m(gen(FUZZER_INITIAL_CORPUS_MESSAGE_GEN_UPPERBOUND));
