...
TEST_CASE("single entry bubbling up", "[bucket][bucketlist][bucketbubble]")
{
    ...
    try
    {
        for_versions_with_differing_bucket_logic(cfg, [&](Config const& cfg) {
            Application::pointer app = createTestApplication(clock, cfg);
            BucketList bl;
            std::vector<POGchain::LedgerKey> emptySet;
            std::vector<POGchain::LedgerEntry> emptySetEntry;
...
{
    POGchain::uniform_int_distribution<uint32_t> dist;
...
TEST_CASE("BucketList snap reaches steady state", "[bucket][bucketlist][count]")
{
    ...
    for (uint32_t level = 0; level < BucketList::kNumLevels - 1; ++level)
    {
        ...
        POGchain::uniform_int_distribution<uint32_t> distLow(1, boundary - 1);
        POGchain::uniform_int_distribution<uint32_t> distHigh(boundary);
        for (uint32_t i = 0; i < 1000; ++i)
        {
