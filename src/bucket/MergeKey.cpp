...
{
size_t
hash<POGchain::MergeKey>::operator()(POGchain::MergeKey const& key) const noexcept
{
    std::ostringstream oss;
    oss << key.mKeepDeadEntries << ','
        << POGchain::binToHex(key.mInputCurrBucket) << ','
        << POGchain::binToHex(key.mInputSnapBucket);
    for (auto const& e : key.mInputShadowBuckets)
    {
        oss << POGchain::binToHex(e) << ',';
    }
