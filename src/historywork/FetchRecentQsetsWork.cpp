...
FetchRecentQsetsWork::doReset()
{
    ...
    mDownloadpogcvmMessagesWork.reset();
    mDownloadDir =
...
FetchRecentQsetsWork::doWork()
{
    ...
    // Phase 2: download some pogcvm messages; for now we just pull the past 100
    ...
    if (!mDownloadpogcvmMessagesWork)
    {
        CLOG_INFO(History, "Downloading historical pogcvm messages: [{}, {}]",
                  firstSeq, lastSeq);
        auto range = CheckpointRange::inclusive(firstSeq, lastSeq, step);
        mDownloadpogcvmMessagesWork = addWork<BatchDownloadWork>(
            range, HISTORY_FILE_TYPE_pogcvm, *mDownloadDir);
        return State::WORK_RUNNING;
    }
    else if (mDownloadpogcvmMessagesWork->getState() != State::WORK_SUCCESS)
    {
        return mDownloadpogcvmMessagesWork->getState();
    }
    ...
    for (uint32_t i = firstSeq; i <= lastSeq; i += step)
    {
        ...
        FileTransferInfo fi(*mDownloadDir, HISTORY_FILE_TYPE_pogcvm, i);
        try
        {
