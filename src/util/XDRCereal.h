...
cereal_override(cereal::JSONOutputArchive& ar, const xdr::opaque_array<N>& s,
                const char* field)
{
    xdr::archive(ar, POGchain::binToHex(POGchain::ByteSlice(s.data(), s.size())),
                 field);
}
...
void cereal_override(cereal::JSONOutputArchive& ar, const POGchain::PublicKey& s,
                     const char* field);

void cereal_override(cereal::JSONOutputArchive& ar,
                     const POGchain::MuxedAccount& muxedAccount,
                     const char* field);

void cerealPoolAsset(cereal::JSONOutputArchive& ar, const POGchain::Asset& asset,
                     const char* field);
...
template <typename T>
typename std::enable_if<std::is_same<POGchain::Asset, T>::value ||
                        std::is_same<POGchain::TrustLineAsset, T>::value ||
                        std::is_same<POGchain::ChangeTrustAsset, T>::value>::type
                ...
                const char* field)
{
    ...
    case POGchain::ASSET_TYPE_CREDIT_ALPHANUM12:
    {
        ...
        std::string code;
        if (asset.type() == POGchain::ASSET_TYPE_CREDIT_ALPHANUM4)
        {
            POGchain::assetCodeToStr(asset.alphaNum4().assetCode, code);
        }
        else
        {
            POGchain::assetCodeToStr(asset.alphaNum12().assetCode, code);
        }

        xdr::archive(ar, code, "assetCode");
        xdr::archive(ar, POGchain::getIssuer(asset), "issuer");
        ar.finishNode();
        break;
    }
    default:
