...
// elsewhere in POGchain so there's a little work up front converting
// representations.
//
...
class QuorumIntersectionCheckerImpl : public POGchain::QuorumIntersectionChecker
{

    POGchain::Config const& mCfg;
    ...
    // reporting.
    mutable std::pair<std::vector<POGchain::NodeID>,
                      std::vector<POGchain::NodeID>>
        mPotentialSplit;
    ...
    std::vector<POGchain::NodeID> mBitNumPubKeys;
    std::unordered_map<POGchain::NodeID, size_t> mPubKeyBitNums;
    QGraph mGraph;
    ...
    mutable POGchain::RandomEvictionCache<BitSet, bool, BitSet::HashFunction>
        mCachedQuorums;
  ...
  public:
    QuorumIntersectionCheckerImpl(POGchain::QuorumTracker::QuorumMap const& qmap,
                                  POGchain::Config const& cfg,
                                  std::atomic<bool>& interruptFlag,
                                  bool quiet = false);
    bool networkEnjoysQuorumIntersection() const override;

    std::pair<std::vector<POGchain::NodeID>, std::vector<POGchain::NodeID>>
