// Copyright 2016-2019 POGchain Development Foundation and contributors. Licensed
...
TEST_CASE("quorum intersection 8-org -and-periphery dangling",
          "[herder][quorumintersection]")
{
    // This configuration "looks kinda strong" -- there's a fully-connected
    // "" org set and the "periphery" orgs are all set to 3/3 between their
    // own two nodes and the  org they're watching -- but it is still
    // capable of splitting in half because the  orgs' dependency on on
    // periphery orgs allows passing the  org's 5/7 threshold without
    // needing a majority of the  orgs. The  orgs can be satisfied by
    // their own 3 nodes + 1 other  node + 1 periphery org, which is enough
    ...
    auto qm = interconnectOrgsBidir(
        orgs,
        {// 4  orgs 0, 1, 2, 3 (with 3 nodes each) which fully depend on one
         // another.
         {0, 1},
         {0, 2},
         {0, 3},
         ...
         // trust with one  org, which is that  org's only paired
         // periphery.
         {0, 4},
         {1, 5},
         {2, 6},
...
TEST_CASE("quorum intersection 8-org -and-periphery balanced",
          "[herder][quorumintersection]")
{
    // This configuration strengthens the previous just a bit by making each of
    // the  orgs have _two_ periphery relationships, in a specific
    // "balanced" pattern of peripheral dependency. The periphery nodes are
    // still able to be satisfied by 3/4 threshold so they can "go with" a 
    // node, but the  nodes have been pushed from 5/7 up to 6/8 which means
    // they need their own 3 nodes + 2 periphery orgs + 1 other 
    // org. Needing two periphery orgs means -- due to the balanced distribution
    // of /periphery relationships -- that one of those periphery orgs spans
    // any possible split across the , which means there's quorum
    ...
    auto qm = interconnectOrgsBidir(
        ...
        {// 4  orgs 0, 1, 2, 3 (with 3 nodes each) which fully depend on one
         ...
         {2, 3},
         // 4 "periphery" orgs (with 2 nodes each), each with bidirectional
         // trust with two  orgs, with each pair of  orgs having only
...
TEST_CASE("quorum intersection 8-org -and-periphery unbalanced",
          "[herder][quorumintersection]")
{
    ...
    auto qm = interconnectOrgsBidir(
        ...
        {// 4  orgs 0, 1, 2, 3 (with 3 nodes each) which fully depend on one
         ...
         {2, 3},
         // 4 "periphery" orgs (with 2 nodes each), each with bidirectional
         // trust with two  orgs, with two pairs of  orgs paired to
...
TEST_CASE("quorum intersection 6-org 1-node 4-null qsets",
          "[herder][quorumintersection]")
{
    // Generating the following topology with dependencies from the  nodes
