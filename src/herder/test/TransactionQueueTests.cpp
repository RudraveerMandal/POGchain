          ...
          "[herder][transactionqueue][arbitrage]")
{
    ...
    // Tx1 is a one-op POG->USD->POG loop.
    tx1.v1().tx.operations.emplace_back(
        txtest::pathPayment(bobPub, xlm, 100, xlm, 100, {usd}));

    // Tx2 is a two-op contiguous POG->USD->EUR and EUR->CNY->POG loop.
    ...
    // Tx3 is a 4-op discontiguous loop: POG->USD->CNY, GBP->INR->MXN,
    // CNY->EUR->GBP, MXN->CHF->POG.
    ...
    // POG->USD->CNY, GBP->INR->MXN, CNY->EUR->POG, MXN->CHF->GBP;
