...
using namespace POGchain::txbridge;
...
makeMultiPayment(POGchain::TestAccount& destAccount, POGchain::TestAccount& src,
                 int nbOps, int64 paymentBase, uint32 extraFee, uint32 feeMult)
{
    std::vector<POGchain::Operation> ops;
...
testpogcvmDriver(uint32 protocolVersion, uint32_t maxTxSize, size_t expectedOps)
{
    ...
    SECTION("combineCandidates")
    {
        ...
        auto addCandidateThenTest = [&](CandidateSpec const& spec) {
            ...
            // increment, and make it into a POGchainValue to add to the list
            ...
            // Compare the returned POGchainValue's contents with the
    ...
    SECTION("validateValue signatures")
    {
        ...
        SECTION("invalid")
        {
            ...
            auto testInvalidValue = [&](bool isNomination) {
                ...
                {
                    auto basicVal =
                        POGchainValue(txSet0->getContentsHash(), ct,
                                     emptyUpgradeSteps, POGchain_VALUE_BASIC);
...
TEST_CASE("In quorum filtering", "[quorum][herder][acceptance]")
{
    ...
    auto sim = Topologies::(4, 0.75, mode, networkID, [](int i) {
        return getTestConfig(i, Config::TESTDB_ON_DISK_SQLITE);
    });

    sim->startAllNodes();

    // first, close ledgers with a simple topology 0..3
    ...
    //  nodes but they still externalize
    ...
    // then, change the quorum set of node 3 to also include "E_2" and "E_3"
