...
class QuorumIntersectionChecker
{
  public:
    ...
    create(POGchain::QuorumTracker::QuorumMap const& qmap,
           POGchain::Config const& cfg, std::atomic<bool>& interruptFlag,
           bool quiet = false);

    static std::set<std::set<NodeID>>
    getIntersectionCriticalGroups(POGchain::QuorumTracker::QuorumMap const& qmap,
