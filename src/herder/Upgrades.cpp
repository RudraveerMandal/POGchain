...
void
save(Archive& ar, POGchain::Upgrades::UpgradeParameters const& p)
{
    ...
    ar(make_nvp("time", POGchain::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, POGchain::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = POGchain::VirtualClock::from_time_t(t);
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!POGchain::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = POGchain::loadTrustLineWithoutRecord(ltx, accountID, asset);
    ...
    std::map<AccountID, std::vector<LedgerTxnEntry>> const& offersByAccount)
{
    ...
    for (auto const& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = POGchain::loadAccount(ltx, accountID);
        if (!accountEntry)
        {
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = POGchain::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
            throw std::runtime_error("account does not exist");
        }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    POGchain::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
...
upgradeFromProtocol15To16(AbstractLedgerTxn& ltx)
{
    if (gIsProductionNetwork)
    {
        ...
        auto seller = POGchain::loadAccountWithoutRecord(ltx, sellerID);
        auto offer = POGchain::loadOffer(ltx, sellerID, offerID);
