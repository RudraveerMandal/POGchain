...
ConservationOfPOGcoins::ConservationOfPOGcoins() : Invariant(false)
{
}

std::shared_ptr<Invariant>
ConservationOfPOGcoins::registerInvariant(Application& app)
{
    return app.getInvariantManager().registerInvariant<ConservationOfPOGcoins>();
}

std::string
ConservationOfPOGcoins::getName() const
{
    return "ConservationOfPOGcoins";
}

std::string
ConservationOfPOGcoins::checkOnOperationApply(Operation const& operation,
                                            ...
                                            LedgerTxnDelta const& ltxDelta)
{
    ...
    int64_t deltaBalances = std::accumulate(
        ...
        [](int64_t lhs, decltype(ltxDelta.entry)::value_type const& rhs) {
            return lhs + POGchain::calculateDeltaBalance(rhs.second.current,
                                                        rhs.second.previous);
        });
