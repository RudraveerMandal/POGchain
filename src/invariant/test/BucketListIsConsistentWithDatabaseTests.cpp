...
struct BucketListGenerator
{
  ...
  public:
    ...
    generateDeadEntries(AbstractLedgerTxn& ltx)
    {
        ...
        {
            auto dist =
                POGchain::uniform_int_distribution<size_t>(0, live.size() - 1);
          ...
          "[invariant][bucketlistconsistent]")
{
    struct TestRootBucketListGenerator : public BucketListGenerator
    {
        ...
        TestRootBucketListGenerator()
            : mTargetLedger(POGchain::uniform_int_distribution<uint32_t>(2, 100)(
                  gRandomEngine))
            , mModifiedRoot(false)
        {
        }
        ...
        generateLiveEntries(AbstractLedgerTxn& ltx)
        {
            if (mLedgerSeq == mTargetLedger)
            {
                ...
                auto le =
                    POGchain::loadAccountWithoutRecord(ltx, root).current();
                le.lastModifiedLedgerSeq = mLedgerSeq;
                return {le};
            }
            else
          ...
          "[invariant][bucketlistconsistent][acceptance]")
{
    auto runTest = [](bool withFilter) {
        for (size_t nTests = 0; nTests < 40; ++nTests)
        {
            ...
            POGchain::uniform_int_distribution<uint32_t> addAtLedgerDist(
                2, blg.mLedgerSeq);
          ...
          "[invariant][bucketlistconsistent][acceptance]")
{
    ...
    for (uint32_t level = 0; level < BucketList::kNumLevels; ++level)
    {
        ...
        POGchain::uniform_int_distribution<uint32_t> ledgerToModifyDist(
        ...
        for (uint32_t i = 0; i < 10; ++i)
        {
            ...
            POGchain::uniform_int_distribution<uint32_t> lowTargetLedgerDist(
                1, maxLowTargetLedger);
            POGchain::uniform_int_distribution<uint32_t> highTargetLedgerDist(
