                      ...
                      public NonMovableOrCopyable
{
  public:
    ...
    void relayOrProcessResponse(POGchainMessage const& msg, Peer::pointer peer);
    void relayOrProcessRequest(POGchainMessage const& msg, Peer::pointer peer);
    void clearOldLedgers(uint32_t lastClosedledgerSeq);
    Json::Value const& getJsonResults();

    static std::string getMsgSummary(POGchainMessage const& msg);
    POGchainMessage makeSurveyRequest(NodeID const& nodeToSurvey) const;
  ...
  private:
    ...
    void broadcast(POGchainMessage const& msg) const;
