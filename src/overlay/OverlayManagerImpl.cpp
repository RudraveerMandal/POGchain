...
{
    POGchain::shuffle(peerList.begin(), peerList.end(), gRandomEngine);
}

bool
OverlayManagerImpl::recvFloodedMsgID(POGchainMessage const& msg,
...
bool
OverlayManagerImpl::broadcastMessage(POGchainMessage const& msg, bool force)
{
    ZoneScoped;
...
void
OverlayManagerImpl::recordMessageMetric(POGchainMessage const& POGchainMsg,
                                        Peer::pointer peer)
{
    ZoneScoped;
    auto logMessage = [&](bool unique, std::string const& msgType) {
        CLOG_TRACE(Overlay, "recv: {} {} ({}) of size: {} from: {}",
                   ...
                   peer->msgSummary(POGchainMsg), msgType,
                   xdr::xdr_argpack_size(POGchainMsg),
                   mApp.getConfig().toShortString(peer->getPeerID()));
    };
    ...
    size_t size = xdr::xdr_argpack_size(POGchainMsg);
    auto hash = shortHash::xdrComputeHash(POGchainMsg);
...
void
OverlayManagerImpl::updateFloodRecord(POGchainMessage const& oldMsg,
