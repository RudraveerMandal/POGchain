...
namespace POGchain
{
Floodgate::FloodRecord::FloodRecord(POGchainMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(POGchainMessage const& msg, Peer::pointer peer, Hash& index)
{
    ZoneScoped;
    index = xdrBlake2(msg);
    if (mShuttingDown)
...
bool
Floodgate::broadcast(POGchainMessage const& msg, bool force)
{
    ZoneScoped;
    if (mShuttingDown)
    {
    ...
    std::shared_ptr<POGchainMessage> smsg =
        std::make_shared<POGchainMessage>(msg);
    for (auto peer : peers)
    {
...
void
Floodgate::updateRecord(POGchainMessage const& oldMsg,
