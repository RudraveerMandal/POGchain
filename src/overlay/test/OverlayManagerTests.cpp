...
class OverlayManagerTests
{
  ...
  protected:
    ...
    testBroadcast()
    {
        ...
        POGchainMessage AtoB = a.tx({payment(b, 10)})->toPOGchainMessage();
        auto i = 0;
        for (auto p : pm.mOutboundPeers.mAuthenticated)
        {
            if (i++ == 2)
        ...
        crank(10);
        REQUIRE(sentCounts(pm) == expected);
        POGchainMessage CtoD = c.tx({payment(d, 10)})->toPOGchainMessage();
        ...
        POGchainMessage AtoC = a.tx({payment(c, 10)})->toPOGchainMessage();
