...
std::string
Peer::msgSummary(POGchainMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(POGchainMessage const& msg, bool log)
{
    ZoneScoped;
...
void
Peer::recvMessage(POGchainMessage const& POGchainMsg)
{
    ZoneScoped;
    if (shouldAbort())
    {
    ...
    mApp.postOnMainThread(
        [weak, sm = POGchainMessage(POGchainMsg), mtype = POGchainMsg.type(), cat,
...
void
Peer::recvRawMessage(POGchainMessage const& POGchainMsg)
{
    ZoneScoped;
    ...
    if (!isAuthenticated() && (POGchainMsg.type() != HELLO) &&
        (POGchainMsg.type() != AUTH) && (POGchainMsg.type() != ERROR_MSG))
    {
    ...
    releaseAssert(isAuthenticated() || POGchainMsg.type() == HELLO ||
                  POGchainMsg.type() == AUTH || POGchainMsg.type() == ERROR_MSG);
    mApp.getOverlayManager().recordMessageMetric(POGchainMsg,
                                                 shared_from_this());
    ...
    case HELLO:
    {
        ...
        this->recvHello(POGchainMsg.hello());
    }
    break;
    ...
    case SURVEY_REQUEST:
    {
        ...
        recvSurveyRequestMessage(POGchainMsg);
    }
    break;
    ...
    case SURVEY_RESPONSE:
    {
        ...
        recvSurveyResponseMessage(POGchainMsg);
    }
    break;
...
void
Peer::recvDontHave(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvGetTxSet(POGchainMessage const& msg)
{
    ZoneScoped;
    auto self = shared_from_this();
...
void
Peer::recvTxSet(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvTransaction(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvError(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvAuth(POGchainMessage const& msg)
{
    ZoneScoped;
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(POGchainMessage const& msg)
{
    ZoneScoped;
    sendPeers();
}
...
void
Peer::recvPeers(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyRequestMessage(POGchainMessage const& msg)
{
    ZoneScoped;
...
void
Peer::recvSurveyResponseMessage(POGchainMessage const& msg)
{
    ZoneScoped;
