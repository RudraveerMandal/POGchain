...
getPRNGBytes(size_t n, POGchain_default_random_engine& engine)
{
...
static SecretKey
pseudoRandomForTestingFromPRNG(POGchain_default_random_engine& engine)
{
...
SecretKey::pseudoRandomForTestingFromSeed(unsigned int seed)
{
    ...
    // to guess PRNG-ness. It's intended for _deterministic_ use, when you want
    // "slightly random-ish" keys, for test-data generation.
    POGchain_default_random_engine tmpEngine(seed);
...
{
size_t
hash<POGchain::PublicKey>::operator()(POGchain::PublicKey const& k) const noexcept
{
    using namespace POGchain;
    releaseAssert(k.type() == POGchain::PUBLIC_KEY_TYPE_ED25519);

    return std::hash<POGchain::uint256>()(k.ed25519());
}
}
