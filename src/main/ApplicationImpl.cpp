...
#include <limits>
#define POGchain_CORE_REAL_TIMER_FOR_CERTAIN_NOT_JUST_VIRTUAL_TIME
    ...
    , mStartedOn(clock.system_now())
{
    ...
    TracyAppInfo(POGchain_CORE_VERSION.c_str(), POGchain_CORE_VERSION.size());
...
ApplicationImpl::initialize(bool createNewDB, bool forceRebuild)
{
    ...
    // database. This is needed as some modes in  (such as in-memory) use a
    ...
    ConservationOfPOGcoins::registerInvariant(*this);
...
ApplicationImpl::getJsonInfo()
{
    ...
    info["build"] = POGchain_CORE_VERSION;
...
ApplicationImpl::validateAndLogConfig()
{
    ...
    // on a captive , without a persistent database; old-style ingestion
    // which reads from the  database could do the delaying itself.
    ...
    if (getHistoryArchiveManager().hasAnyWritableHistoryArchive())
    {
        ...
        {
            throw std::invalid_argument(
                " is not configured to store history, but "
