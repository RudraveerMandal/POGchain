...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"POGchain.cfg"} : mConfigFile;
        ...
        [&] {
            ...
            try
            {
                ...
                if (cfg.MANUAL_CLOSE)
                {
                    ...
                    {
                        LOG_ERROR(DEFAULT_LOG, "Starting POGchain in "
...
{
    std::cout << POGchain_CORE_VERSION << std::endl;
    return 0;
}
