...
{
    // Number of streaming ledgers ahead of LCL.  will
         ...
         std::string const& startAtHash)
{
    ...
    LOG_INFO(DEFAULT_LOG, "Starting POGchain {}", POGchain_CORE_VERSION);
    Application::pointer app;
...
{
    // Certain in-memory modes in  may start the app before reaching this
    ...
    {
        LOG_FATAL(DEFAULT_LOG, "Got an exception: {}", e.what());
        throw; // propagate exception ( dump, etc)
    }
    return 0;
}
        ...
        Json::Value& catchupInfo, std::shared_ptr<HistoryArchive> archive)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG_INFO(
            DEFAULT_LOG,
            "* If you really want to catchup to {} run POGchain new-db",
            cc.toLedger());
        LOG_INFO(DEFAULT_LOG, "*");
        return 2;
    }
