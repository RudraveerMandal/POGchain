...
TEST_CASE("resolve node id", "[config]")
{
    ...
    cfg.VALIDATOR_NAMES.emplace(std::make_pair(validator1Key, "-testnet1"));
    cfg.VALIDATOR_NAMES.emplace(std::make_pair(validator2Key, "-testnet2"));
    cfg.VALIDATOR_NAMES.emplace(std::make_pair(validator3Key, "-testnet3"));
    ...
    {
        auto publicKey = PublicKey{};
        auto result = cfg.resolveNodeID("$-testnet1", publicKey);
        REQUIRE(result);
        REQUIRE(keyMatches(publicKey, {validator1Key}));
    }
    ...
    {
        auto publicKey = PublicKey{};
        auto result = cfg.resolveNodeID("$", publicKey);
        REQUIRE(!result);
    }
...
{
    Config c;
    c.load("testdata/POGchain_example_validators.cfg");
...
TEST_CASE("load example configs", "[config]")
{
    ...
    std::vector<std::string> testFiles = {
        "POGchain_example.cfg", "POGchain_standalone.cfg",
        "POGchain_testnet_legacy.cfg", "POGchain_testnet.cfg",
        "POGchain_testnet_validator.cfg"};
...
TEST_CASE("operation filter configuration", "[config]")
{
    ...
    POGchain::uniform_int_distribution<size_t> dist(
    ...
    for (size_t i = 0; i < 5; ++i)
    {
        ...
        POGchain::shuffle(vals.begin(), vals.end(), gRandomEngine);
