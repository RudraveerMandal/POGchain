...
using namespace POGchain::historytestutils;
    ...
    damageVictim() override
    {
                      ...
                      /*shouldUpdateLastModified=*/false);
        {
            auto rootKey = accountKey(
                POGchain::txtest::getRoot(app->getNetworkID()).getPublicKey());
...
TEST_CASE("application setup", "[applicationutils]")
{
    ...
    SECTION("in memory mode")
    {
        ...
        // Captive  points to a read-only archive maintained by the
        // validator
        ...
        // Shutdown captive  and test various startup paths
