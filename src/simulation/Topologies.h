...
class Topologies
{
  public:
    ...
    static Simulation::pointer
    (int nNodes, double quorumThresoldFraction, Simulation::Mode mode,
    ...
    // multi-tier quorum (4 + mid-tier nodes that depend on 2 nodes of
    // 4) mid-tier connected round-robin to 4
    static Simulation::pointer hierarchicalQuorum(
        int nBranches, Simulation::Mode mode, Hash const& networkID,
        Simulation::ConfigGen confGen = nullptr, int connectionsTo = 1,
        Simulation::QuorumSetAdjuster qSetAdjust = nullptr);

    // 2-tier quorum with a variable size  (with 0.75 threshold)
    // and outer-nodes that listen to  & self
    // outer-nodes have connectionsTo connections to  nodes
    // (round-robin)
    ...
    // Asymmetric modifies `` topology by adding extra nodes to one of the
    // validators in 
    static Simulation::pointer
