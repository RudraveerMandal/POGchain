...
TEST_CASE("3 nodes 2 running threshold 2", "[simulation][3][acceptance]")
{
    ...
    LOG_DEBUG(DEFAULT_LOG, "done with 3 test");
}
...
TEST_CASE(" topology 4 ledgers at scales 2 to 4",
          "[simulation][acceptance]")
{
    ...
    {
        Simulation::pointer sim = Topologies::(size, 1.0, mode, networkID);
        sim->startAllNodes();
...
resilienceTest(Simulation::pointer sim)
{
    ...
    POGchain::uniform_int_distribution<size_t> gen(0, nbNodes - 1);
...
TEST_CASE("resilience tests", "[resilience][simulation][!hide]")
{
    ...
    SECTION("4")
    {
        resilienceTest(Topologies::(4, 0.75, mode, networkID, confGen));
    }
...
static void
hierarchicalSimplifiedTest(int nLedgers, int nb, int nbOuterNodes,
                           Simulation::Mode mode, Hash const& networkID)
{
    ...
    Simulation::pointer sim = Topologies::hierarchicalQuorumSimplified(
        nb, nbOuterNodes, mode, networkID);
    sim->startAllNodes();
...
TEST_CASE(" nodes with outer nodes", "[simulation][acceptance]")
{
    Hash networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
    Simulation::Mode mode = Simulation::OVER_LOOPBACK;
    SECTION("Over loopback")
