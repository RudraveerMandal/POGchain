                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = POGchain::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return POGchain::loadAccount(ltx, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerTxn& ltx, int64_t baseFee)
{
    ...
    if (fee > 0)
    {
        ...
        POGchain::addBalance(acc.balance, -fee);
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    auto account = POGchain::loadAccount(ltx, accountID);
    if (!account)
    {
...
POGchainMessage
TransactionFrame::toPOGchainMessage() const
{
    POGchainMessage msg(TRANSACTION);
