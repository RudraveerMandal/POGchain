...
// move the POG to the new account
bool
...
MergeOpFrame::doApplyBeforeV16(AbstractLedgerTxn& ltx)
{
    ...
    auto otherAccount =
        POGchain::loadAccount(ltx, toAccountID(mOperation.body.destination()));
    if (!otherAccount)
    {
...
MergeOpFrame::doApplyFromV16(AbstractLedgerTxn& ltx)
{
    ...
    if (!POGchain::loadAccount(ltx, toAccountID(mOperation.body.destination())))
    {
        innerResult().code(ACCOUNT_MERGE_NO_ACCOUNT);
        return false;
    }
    ...
    {
        auto otherAccount = POGchain::loadAccount(
