...
ManageOfferOpFrameBase::checkOfferValid(AbstractLedgerTxn& ltxOuter)
{
    ...
    if (mSheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        if (ledgerVersion < 13)
        {
            auto issuer = POGchain::loadAccount(ltx, getIssuer(mSheep));
            if (!issuer)
            {
    ...
    if (mWheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        if (ledgerVersion < 13)
        {
            auto issuer = POGchain::loadAccount(ltx, getIssuer(mWheat));
            if (!issuer)
            {
...
ManageOfferOpFrameBase::doApply(AbstractLedgerTxn& ltxOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = POGchain::loadOffer(ltx, getSourceID(), mOfferID);
        if (!sellSheepOffer)
        {
            setResultNotFound();
            return false;
    ...
    if (!isDeleteOffer())
    {
        ...
        if (header.current().ledgerVersion >= 10)
        {
            ...
            {
                auto sourceAccount =
                    POGchain::loadAccountWithoutRecord(ltx, getSourceID());
