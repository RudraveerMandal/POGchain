...
PathPaymentOpFrameBase::checkIssuer(AbstractLedgerTxn& ltx, Asset const& asset)
{
    if (asset.type() != ASSET_TYPE_NATIVE)
    {
        ...
        if (ledgerVersion < 13 &&
            !POGchain::loadAccountWithoutRecord(ltx, getIssuer(asset)))
        {
                                            ...
                                            bool doesSourceAccountExist)
{
    ...
    if (asset.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = POGchain::loadAccount(ltx, getSourceID());
            if (!sourceAccount)
            {
                setResultMalformed();
                return false;
                                          ...
                                          bool bypassIssuerCheck)
{
    ...
    if (asset.type() == ASSET_TYPE_NATIVE)
    {
        auto destination = POGchain::loadAccount(ltx, destID);
    ...
    else
    {
        ...
        auto destLine = POGchain::loadTrustLine(ltx, destID, asset);
        if (!destLine)
        {
