...
FeeBumpTransactionFrame::commonValidPreSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (!POGchain::loadAccount(ltx, getFeeSourceID()))
    {
                                     ...
                                     AbstractLedgerTxn& ltxOuter, bool applying)
{
    ...
    auto feeSource = POGchain::loadAccount(ltx, getFeeSourceID());
    if (!checkSignature(
...
{
    auto account = POGchain::loadAccount(ltx, getFeeSourceID());
    if (!account)
    {
...
POGchainMessage
FeeBumpTransactionFrame::toPOGchainMessage() const
{
