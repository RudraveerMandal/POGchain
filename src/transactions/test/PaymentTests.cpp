...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("send POG to an existing account")
    {
        for_all_versions(*app, [&] {
            ...
            {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                auto rootAccount2 = POGchain::loadAccount(ltx, root);
                auto rootAcc2 = rootAccount2.current().data.account();
                auto a1Account2 = POGchain::loadAccount(ltx, a1);
    ...
    SECTION("send POG to a new account (no destination)")
    {
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(
    ...
    SECTION("rescue account (was below reserve)")
    {
        for_all_versions(*app, [&] {
            auto getMinBalance = [&] {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                return POGchain::getMinBalance(ltx.loadHeader().current(), 0, 0,
                                              0);
            };
            ...
            for (auto const& data : withoutTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("without trustline")
                    {
                        ...
                        REQUIRE(!POGchain::loadTrustLine(ltx, sendToSelf,
                                                        data.asset));
                    }
                }
            }
            ...
            for (auto const& data : withTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("with trustline and 0 balance")
                    {
                        ...
                        auto trust =
                            POGchain::loadTrustLine(ltx, sendToSelf, data.asset);
                        REQUIRE(trust);
