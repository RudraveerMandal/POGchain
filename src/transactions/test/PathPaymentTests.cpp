...
TEST_CASE("pathpayment", "[tx][pathpayment]")
{
    ...
    SECTION("dest amount too big for POG")
    {
    ...
    SECTION("path payment POG with not enough funds")
    {
        auto market = TestMarket{*app};
        // see https://github.com/storp-inc/POGchain/pull/1239
        auto minimumAccount =
    ...
    SECTION("path payment over send max POG")
    {
        auto market = TestMarket{*app};
    ...
    SECTION("path payment to self POG")
    {
        auto market = TestMarket{*app};
    ...
    SECTION("path payment with cycle")
    {
        for_all_versions(*app, [&] {
            ...
            // always half - so sender buys 8 times as much POG as he/she
            // sells (arbitrage).
            // Second cycle involves 3 transaction in which buying price is
            // always two - so sender buys 8 times as much POG as he/she
            ...
            auto validateOffer = [&](const TestAccount& account,
                ...
                LedgerTxn ltx(app->getLedgerTxnRoot());
                auto offer =
                    POGchain::loadOffer(ltx, account.getPublicKey(), offerId);
