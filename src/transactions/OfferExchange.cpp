           ...
           std::vector<ClaimAtom>& offerTrail)
{
    ...
    {
        auto accountB = POGchain::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        auto accountB = POGchain::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = POGchain::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                POGchain::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                POGchain::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              RoundingType round, std::vector<ClaimAtom>& offerTrail)
{
    ...
    if (!POGchain::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = POGchain::loadAccount(ltx, accountBID);
    }
