// Copyright 2021 POGchain Development Foundation and contributors. Licensed
              ...
              LedgerTxnEntry& acc, int64_t delta)
{
    ...
    {
        if (!POGchain::addBalance(header, acc, delta))
        {
            throw std::runtime_error("insufficient balance");
        }
    }
...
LiquidityPoolDepositOpFrame::doApply(AbstractLedgerTxn& ltx)
{
    ...
    if (!POGchain::addBalance(cp().reserveA, amountA, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }
    updateBalance(header, tlB, source, -amountB);
    if (!POGchain::addBalance(cp().reserveB, amountB, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }

    if (!POGchain::addBalance(header, tlPool, amountPoolShares))
    {
        throw std::runtime_error("insufficient pool share limit");
    }
    if (!POGchain::addBalance(cp().totalPoolShares, amountPoolShares, INT64_MAX))
    {
        throw std::runtime_error("insufficient liquidity pool limit");
    }
