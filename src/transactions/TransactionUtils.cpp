                            ...
                            LedgerTxnEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ltx, &sellerID]() {
        auto account = POGchain::loadAccount(ltx, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ltx, &sellerID](Asset const& asset) {
        auto trust = POGchain::loadTrustLine(ltx, sellerID, asset);
        if (!trust)
        {
                            ...
                            LedgerTxnEntry& entry, int64_t amount)
{
    ...
    if (!POGchain::addBalance(newBalance, amount, tl.limit))
    {
        return false;
    }
...
addBalance(LedgerTxnHeader const& header, LedgerTxnEntry& entry, int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!POGchain::addBalance(newBalance, delta))
        {
            return false;
        }
        if (header.current().ledgerVersion >= 10)
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = POGchain::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = POGchain::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
                              ...
                              Asset const& asset)
{
    ...
    for (auto& offer : offers)
    {
        ...
        auto trustAcc = POGchain::loadAccount(ltxInner, account);
                                   ...
                                   SequenceNumber txSeqNum, uint32_t opIndex)
{
    ...
    for (auto const& poolTLKey : poolTLKeys)
    {
        ...
        {
            auto trustAcc = POGchain::loadAccount(ltxInner, accountID);
        ...
        auto redeemIntoClaimableBalance =
            ...
            if (loadSponsorship(ltxInner, cbSponsoringAccID))
            {
                auto cbSponsoringLtxAcc =
                    POGchain::loadAccount(ltxInner, cbSponsoringAccID);
...
struct MuxChecker
{
    ...
    void
    operator()(POGchain::MuxedAccount const& t)
    {
